...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function getRatingsPublic() {
    const {
      limit: _limit,
      offset: _offset,
      order,
    ...
    const findOptions = convertQueryToPrismaUtil(query, 'rating');
    const countOptions = omit(findOptions, 'include');
    const [totalCount, data] = await prisma.$transaction([
      prisma.rating.count(countOptions as unknown),
  ...
  async function getRatings() {
    ...
    const response = await prisma.rating
      ...
      .findManyPaginated({
        ...convertQueryToPrismaUtil(query, 'rating'),
        take: limit,
        skip: offset,
        ...(order?.length && {
          orderBy: getOrderByOptions(order),
