...
type ColumnType = ColumnDef<RatingInterface, unknown>;
...
export function RatingListPage(props: RatingListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getRatings({
        ...
        order: params.order,
        searchTermKeys: ['review.contains'],
        ...(params.filters || {}),
      }),
    [params.pageSize, params.pageNumber, params.searchTerm, params.order, params.filters],
  );

  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<RatingInterface>>(
    () => `/ratings?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: RatingInterface) => {
    if (hasAccess('rating', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/ratings/view/${row.id}`);
    }
  };

  const columns: ColumnType[] = [
    { id: 'rating_rating', header: 'rating_rating', accessorKey: 'rating_rating' },
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/ratings/view/${record.id}`} passHref legacyBehavior>
                <Button
                  onClick={(e) => e.stopPropagation()}
                  mr={2}
                  padding="0rem 8px"
