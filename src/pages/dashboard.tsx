...
import { getRatings } from 'apiSdk/ratings';
...
import { CustomerProfileListPage as ShowCaseList } from 'pages/customer-profiles';

function DashboardPage() {
  const { session } = useSession();

  const ratingStat = useSWR<PaginatedInterface<RatingInterface>>(
    () => `/ratings`,
    () => getRatings({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Ratings',
      result: ratingStat?.data?.totalCount,
      isLoading: ratingStat.isLoading,
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`customer-profiles`} passHref legacyBehavior>
                  <Button
